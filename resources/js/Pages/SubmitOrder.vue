  ...
  <div class="container mt-4">
    <div class="table-responsive">
      <table class="table table-striped table-bordered">
        ...
        <tbody>
          <!-- Loop through grouped orders by provider -->
          <template v-for="(orders, providerName) in submittedOrders" :key="providerName">
            <!-- Loop through each order for the provider -->
            <tr v-for="(order, orderIndex) in orders" :key="orderIndex">
              <!-- Display provider name only for the first row of each provider -->
              <td v-if="orderIndex === 0" :rowspan="orders.reduce((acc, o) => acc + o.items.length, 0)">
                {{ providerName }}
              </td>
              <td>{{ order.items[0].item_name }}</td>
              <td>{{ order.items[0].unit_price }}</td>
              <td>{{ order.items[0].quantity }}</td>
              <td>{{ order.items[0].subtotal }}</td>
            </tr>
            <!-- Loop through remaining items for the current order -->
            <tr v-for="(item, itemIndex) in order.items.slice(1)" :key="itemIndex">
  ...
  methods: {
    fetchOrders() {
      fetch('/api/orders')
        ...
        .then((data) => {
          if (data.success) {
            this.submittedOrders = data.data;
          } else {
